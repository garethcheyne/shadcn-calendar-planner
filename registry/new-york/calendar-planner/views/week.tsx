...
export interface WeekViewProps<TEvent extends CalendarEvent = CalendarEvent> {
  date: Date
  events: TEvent[]
...
export function WeekView<TEvent extends CalendarEvent = CalendarEvent>({
  date,
  events,
  backgroundEvents,
  step,
  ...
  const range = useMemo(() => {
    ...
    return dates.range(start, end, "day")
  }, [date, localizer])
...
WeekView.range = (date: Date, localizer: DateLocalizer) => {
  const start = localizer.startOf(date, "week")
  const end = localizer.endOf(date, "week")
  return { start, end }
}

WeekView.navigate = (date: Date, action: NavigateAction, localizer: DateLocalizer) => {
  switch (action) {
...
WeekView.title = (date: Date, localizer: DateLocalizer) => {
  const start = localizer.startOf(date, "week")
  const end = localizer.endOf(date, "week")
